...
export default function Footer() {
  return (
    <footer className="relative overflow-hidden" style={{ borderTop: '1px solid rgba(6,182,212,0.1)' }}>
      {/* Ambient glow */}
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none overflow-hidden">
        <span className="text-[8rem] lg:text-[14rem] font-black tracking-tighter whitespace-nowrap"
          style={{ color: 'rgba(34,211,238,0.025)' }}>
          ANONDO
        </span>
      </div>
            ...
            <div className="flex gap-3">
              {socials.map(({ icon: Icon, href, label }) => (
                <motion.a key={label} href={href} aria-label={label} target="_blank" rel="noopener noreferrer"
                  ...
                  className="p-3 rounded-2xl text-slate-400 hover:text-slate-100 transition-colors"
